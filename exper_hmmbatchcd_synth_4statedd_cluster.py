...
from experiment import ExperimentMosix as ExpMosix
...
def main(name, datadir, datafn, script, expdir=None, nfolds=1, nrestarts=1, seed=None):
    """ Run experiment on 4 state, two group synthetic data.
        ...
        script : Path to script to run on cluster.
    ...
    dpath = os.path.join(dataprefix, dpath)
    if len(masks) > 0:
        masks = [os.path.join(dataprefix, mp) for mp in masks]
    ...
    exp = ExpMosix(name, dpath, script, par_list, 'hmmbatchcd',
...
if __name__ == "__main__":
    ...
    name = 'exper_synth_4statedd_cluster'
    datadir = 'data'
    datafn = 'synth_4statedd_T100'
    script = '../run_cluster_simple.py'
    hmmtype = 'hmmbatchcd'
    expdir = None
    nfolds = 3
    nrestarts = 5
    ...
    main(name, datadir, datafn, script, expdir, nfolds, nrestarts, seed)
