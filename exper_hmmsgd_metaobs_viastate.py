...
if __name__ == "__main__":
    """ This test has one state, state 1, that every state must pass through in
        order to get to any other state. For example if state 2 wanted to get
        to state 3, the sequence must got 2 -> 1 -> 3. If our algorithm doesn't
        observe the transition to state 1 then it might think the states are
        transitioning between each other.
    """
    name = 'exper_hmmsgd_metaobs_synth_viastate'
    datadir = 'data'
    datafn = 'synth_viastate_T100'
    expdir = None
    nfolds = 3
    nrestarts = 2
    seed = 8675309
