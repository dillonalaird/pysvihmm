...
if __name__ == "__main__":
    """ This creates a state sequence with two groups, in this case states 1
        and 2 form one group while states 5 and 6 form another group. These
        groups are connected by two one-way bridges. One bridge connects states
        2 -> 3 -> 4 -> 5, the other 6 -> 7 -> 8 -> 1. The point of this
        experiment is to make sure hmmsgd_metaobs observes the bridges. If it
        doesn't see the bridges, then it might incorrectly infer that these
        two groups are transition between eachother. For an interesting
        modified version of this test we could make the means of the two groups
        very close, so the only way to know we've hit state 5, say, is to first
        observe a transition from states 2 -> 3 -> 4 -> 5. This makes
        observing the bridge essential to figuring out the correct transition
        matrix.

        NOTE: We could get rid of 7 or 8 and 3 or 4 and I don't think this
        would change anything.
    """
    name = 'exper_hmmsgd_metaobs_synth_bridge'
    datadir = 'data'
    datafn = 'synth_bridge_T100'
    expdir = None
    nfolds = 3
    nrestarts = 2
    seed = 8675309
